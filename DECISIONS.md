...
## 41. Redirect targets are now a checked surface

Three legacy rules shipped pointing at pages that were never built — `/portfolio/` →
`/work/`, `/privacy-policy/` → `/privacy/`, and both feed URLs → `/rss.xml`. Live, each
answered 301 and then 404.

That is strictly worse than having no rule. A 404 tells a crawler the URL is gone and the
old page's history is written off cleanly; a 301 into a 404 spends the redirect, wastes the
crawl, and leaves the destination looking broken to anyone who followed a link.

The cause was a gap in coverage rather than an oversight in review: `check-links.mjs`
crawled `dist/` for `href` attributes, so every link in the markup was verified while
`public/_redirects` — generated by a different script, never rendered into HTML — was
checked by nothing. It now parses the generated rules and fails on any target absent from
disk, skipping splats and the root. Negative-tested with a deliberately broken rule.

`/privacy-policy/` has no rule at all now rather than a substitute destination. It returns
with the page, in the same commit.

## 42. The lead endpoint is a plain function with two possible mounts

Decision 16 recorded this project as a Worker with static assets, where `functions/` does
nothing. The evidence now contradicts that: `powersourcedigital.pages.dev` serves the
...
**Ordering: KV first, mail second.** The lead is written to storage before the email is
attempted, and the visitor sees success if *either* succeeded. Mail is the failure-prone
half — keys rotate, domains drop out of verification, providers have outages — and a form
that loses a customer to an expired API key is worse than no form, because nobody notices
it is broken.

**Turnstile fails open on a network error** but closed on a missing token. Rejecting real
enquiries whenever Cloudflare's verification endpoint is slow costs more than the spam it
would prevent. Verification is skipped entirely while `TURNSTILE_SECRET_KEY` is unset, so
the form works before the keys exist — recorded in SETUP.md as the thing to fix first.

**No new dependencies.** Validation reuses `normaliseSaudiMobile` and the option lists
already in `src/i18n/forms.ts`, so the accepted values cannot drift from the rendered
`<select>`. Zod would have added a runtime dependency to check six fields; the KV binding is
typed with a six-line local interface rather than the ~1MB `@cloudflare/workers-types`.

**Correction to `.env.example`:** it claimed an unset `LEAD_EMAIL_KEY` falls back to
MailChannels. It does not. MailChannels withdrew free sending from Cloudflare Workers in
June 2024, so there is no keyless mail path. Without the key, leads reach KV and nothing
else.
